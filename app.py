...
# 以下の場合は600秒(10分)
_OBSERVE_NORMAL_TIME = 20 + time.perf_counter()
...
def check_blink(logger:BlinkCounter, app_process):
    global last_calculate_time
    global Tolerance
    last_calculate_time = time.perf_counter()
    if logger.blink_log and len(logger.interval_to_before) >= 2:
        print("check")
        ...
        if Tolerance < abs(variance(logger.interval_to_before) - normal_dispersion):
            popup.popup(app_process)
        

app = Flask(__name__)

def run_camera(app_process):
    global normal_dispersion
    global last_blink_time
    global last_calculate_time
    global Tolerance
    ...
    while True:
        ...
        if _OBSERVE_NORMAL_TIME < time.perf_counter() and len(log.interval_to_before) >= 2 :
            ...
            if time.perf_counter() - last_calculate_time > _INTERVAL:
                ...
                check_blink(logger=log, app_process=app_process)
...
def run_script(app_process):
    # ボタンがクリックされたときに実行するコードをここに書きます
    run_camera(app_process)
